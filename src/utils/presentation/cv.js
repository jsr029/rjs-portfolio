export const cv = [
    {
        header: {
            adresse: "13C rue Charles Gounod",
            lieu: "29000 Quimper",
            tel: "07.69.72.84.73"
        },
        contact: {
            email: "jsr059@gmail.com",
            webSite: "https://rjs-portfolio-tly2.vercel.app/"
        },
        objectif: {
            title: "Objectif",
            name: "JEAN-SEBASTIEN RAKOTONIRINA",
            text: "Autodidacte passionné et motivé, issu d'un niveau BAC+2 en chimie et " +
                "fraichement sorti d'une formation de Développeur d'Applications Front-End ReactJs, " +
                "je suis à la recherche d'un poste de Déveeloppeur dans le domaine du web, idéalement 100% à distance."
        },
        competences: [
            {
                id: 1,
                title: "Compétences",
                subtitle: "Développeur Web",
                items: [
                    "Analyser les besoins du client",
                    "Réaliser des maquettes",
                    "Concevoir et développer les programmes",
                    "Analyser les problèmes techniques",
                    "Déterminer les mesures correctives",
                    "Réaliser les supports techniques"
                ]
            },
            {
                id: 2,
                title: "Compétences",
                subtitle: "Commerce",
                items: [
                    "Organiser une unité de vente",
                    "Agencer un espace de vente",
                    "Valoriser un produit",
                    "Suivre l'état des stocks",
                    "Préparer les commandes",
                    "Réceptionner un produit",
                    "Vérifier la conformité de la livraison",
                    "Stocker un produit"
                ]
            },
            {
                id: 3,
                title: "Compétences",
                subtitle: "Electromécanique",
                items: [
                    "Appliquer les procédures de test",
                    "Sélectionner le matériel de contrôle ou d'essais et vérifier sa conformité",
                    "Simuler les fonctionnalités d'un appareil",
                    "Vérifier le fonctionnement d'un appareil",
                    "Eprouver un appareil dans des conditions extrêmes"
                ]
            },
            {
                id: 4,
                title: "Compétences",
                subtitle: "Commerce",
                items: [
                    "Réaliser la mise en rayon",
                    "Entretenir un espace de vente",
                    "Réaliser le comptage des fonds de caisse",
                    "Accueillir les personnes",
                    "Proposer un service/produit adapté à la demande du client",
                    "Contractualiser une vente",
                    "Réaliser une gestion administrative"
                ]
            },
            {
                id: 5,
                title: "Compétences",
                subtitle: "Administrateur BDD",
                items: [
                    "Gérer l'interface administarteur de l'hébergeur (OVH, 1&1)",
                    "Mettre en place la base via PhpMyAdmin (sql)",
                    "Traiter l'information (collecter, classer et mettre à jour)",
                    "Mettre en place les procédures techniques, d'utilisation et de sécurité des équipements informatiques",
                    "Gérer les droits d'accès des utilisateurs"
                ]
            },
            {
                id: 6,
                title: "Compétences",
                subtitle: "OpenClassRooms",
                items: [
                    "Intégrer du contenu conformément à une maquette",
                    "Implémenter une interface responsive",
                    "Programmer en Javascript",
                    "Analyser un problème informatique",
                    "Développer un algorithme pour résoudre un problème",
                    "Ecrire des test unitaires et d'intégration avec javascript",
                    "Rédiger un plan de test end-to-end manuel",
                    "Développer les routes d'une application web avec React Router",
                    "Initialiser une application web avec un framework",
                    "Créer des composants React..."
                ]
            }
        ],
        xperiences: [
            {
                id:1,
                title: "Expériences Professionelles",
                jobTitle: "Développeur d'Applications web et windows",
                startDate: "2016",
                endDate: "2018",
                lieu: "Dunkerque",
                jobLocation: "Affable59",
                descriptions: [
                    "Mettre en place le site web",
                    "Créer une application windows via Filemaker Pro",
                    "Mettre en place Filemaker Server",
                    "Créer un VPN via Freebox",
                    "Définir le niveau d'accès de chaque Administré"
                ]
            }
        ]
    }
]